/* global streamerGetHexLum, jQuery */
( function() {
	// Add listener for the "background_color" control.
	wp.customize( 'background_color', function( value ) {
		value.bind( function( to ) {
			var lum = streamerGetHexLum( to ),
				isDark = 127 > lum,
				...
				stylesheetID = 'streamer-customizer-inline-styles',
				stylesheet,
				styles;
			...
			if ( ! stylesheet.length ) {
				jQuery( '#streamer-style-inline-css' ).after( '<style id="' + stylesheetID + '"></style>' );
				stylesheet = jQuery( '#' + stylesheetID );
			}
