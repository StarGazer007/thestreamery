 ...
 * @return array
 */
function streamer_body_classes( $classes ) {
...
add_filter( 'body_class', 'streamer_body_classes' );
 ...
 * @return array
 */
function streamer_post_classes( $classes ) {
	$classes[] = 'entry';

	return $classes;
}
add_filter( 'post_class', 'streamer_post_classes', 10, 3 );
 ...
 * @return void
 */
function streamer_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}
add_action( 'wp_head', 'streamer_pingback_header' );
 ...
 * @return void
 */
function streamer_supports_js() {
	echo '<script>document.body.classList.remove("no-js");</script>';
}
add_action( 'wp_footer', 'streamer_supports_js' );
 ...
 * @return array
 */
function streamer_comment_form_defaults( $defaults ) {
...
add_filter( 'comment_form_defaults', 'streamer_comment_form_defaults' );
 ...
 * @return bool
 */
function streamer_can_show_post_thumbnail() {
	return apply_filters(
		'streamer_can_show_post_thumbnail',
 ...
 * @return int
 */
function streamer_get_avatar_size() {
	return 60;
}
 ...
 */
function streamer_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'streamer' ),
 ...
 */
function streamer_continue_reading_link_excerpt() {
	if ( ! is_admin() ) {
		return '&hellip; <a class="more-link" href="' . esc_url( get_permalink() ) . '">' . streamer_continue_reading_text() . '</a>';
	}
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'streamer_continue_reading_link_excerpt' );

/**
 * Create the continue reading link.
 */
function streamer_continue_reading_link() {
	if ( ! is_admin() ) {
		return '<div class="more-link-container"><a class="more-link" href="' . esc_url( get_permalink() ) . '#more-' . esc_attr( get_the_ID() ) . '">' . streamer_continue_reading_text() . '</a></div>';
	}
}

// Filter the excerpt more link.
add_filter( 'the_content_more_link', 'streamer_continue_reading_link' );

if ( ! function_exists( 'streamer_post_title' ) ) {
	/**
	 ...
	 * @return string
	 */
	function streamer_post_title( $title ) {
		return '' === $title ? esc_html_x( 'Untitled', 'Added to posts and pages that are missing titles', 'streamer' ) : $title;
	}
}
add_filter( 'the_title', 'streamer_post_title' );
 ...
 * @return string
 */
function streamer_get_icon_svg( $group, $icon, $size = 24 ) {
	return streamer_SVG_Icons::get_svg( $group, $icon, $size );
}
 ...
 * @return string
 */
function streamer_change_calendar_nav_arrows( $calendar_output ) {
	$calendar_output = str_replace( '&laquo; ', is_rtl() ? streamer_get_icon_svg( 'ui', 'arrow_right' ) : streamer_get_icon_svg( 'ui', 'arrow_left' ), $calendar_output );
	$calendar_output = str_replace( ' &raquo;', is_rtl() ? streamer_get_icon_svg( 'ui', 'arrow_left' ) : streamer_get_icon_svg( 'ui', 'arrow_right' ), $calendar_output );
	return $calendar_output;
}
add_filter( 'get_calendar', 'streamer_change_calendar_nav_arrows' );
 ...
 * @return string
 */
function streamer_get_non_latin_css( $type = 'front-end' ) {
	...
	$font_family = apply_filters(
		'streamer_get_localized_font_family_types',
		array(
	...
	$elements = apply_filters(
		'streamer_get_localized_font_family_elements',
		array(
	...
	if ( ! function_exists( 'streamer_generate_css' ) ) {
	...
	return streamer_generate_css( // @phpstan-ignore-line.
 ...
 */
function streamer_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
 ...
 */
function streamer_password_form( $post = 0 ) {
	$post   = get_post( $post );
	$label  = 'pwbox-' . ( empty( $post->ID ) ? wp_rand() : $post->ID );
	$output = '<p class="post-password-message">' . esc_html__( 'This content is password protected. Please enter a password to view.', 'streamer' ) . '</p>
	<form action="' . esc_url( site_url( 'wp-login.php?action=postpass', 'login_post' ) ) . '" class="post-password-form" method="post">
	<label class="post-password-form__label" for="' . esc_attr( $label )
	. '">' . esc_html_x( 'Password', 'Post password form', 'streamer' )
	. '</label><input class="post-password-form__input" name="post_password" id="'
	. esc_attr( $label ) . '" type="password" size="20" /><input type="submit" class="post-password-form__submit" name="'
	. esc_attr_x( 'Submit', 'Post password form', 'streamer' ) . '" value="' . esc_attr_x( 'Enter', 'Post password form', 'streamer' ) . '" /></form>
	';
	return $output;
}
add_filter( 'the_password_form', 'streamer_password_form' );
 ...
 * @return array
 */
function streamer_get_attachment_image_attributes( $attr, $attachment, $size ) {
...
add_filter( 'wp_get_attachment_image_attributes', 'streamer_get_attachment_image_attributes', 10, 3 );
